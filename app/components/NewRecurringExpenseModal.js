...
export default function NewRecurringExpenseModal() {
  ...
  const newRecurringExpenseModalState = useSelector((state) => state.newRecurringExpenseModal);
  ...
  return (
        <Dialog.Container visible={newRecurringExpenseModalState.open}>
          ...
          <Dialog.Button label="Cancel" onPress={() => dispatch(setNewRecurringExpenseModal(false))} />
        </Dialog.Container>
  );
}
